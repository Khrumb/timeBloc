...
var login = {
  ...
  init:function functionName() {
      ...
      var today = new Date();
			if(err == null){
	      console.log("["+today.toLocaleTimeString()+"][Login Database Log] Database connected.");
        login.db.on('error', function(err) {
          if(err != null){
            console.log("["+today.toLocaleTimeString()+"][Login Database Log] Database disconnected.");
            login.init();
          } else {
            console.log("["+today.toLocaleTimeString()+"][Login Database Log] Unknown error.");
          }
        });
	    } else {
	      console.log("["+today.toLocaleTimeString()+"][Login Database Log] Database Failed to Initialize.");
	    }
		});
  },
...
var content = {
    ...
    init:function functionName() {
        ...
        var today = new Date();
				if(err == null){
			    console.log("["+today.toLocaleTimeString()+"][Content Database Log] Database connected.");

          content.db.on('error', function(err) {
            if(err != null){
              console.log("["+today.toLocaleTimeString()+"][Content Database Log] Database disconnected.");
              content.init();
            } else {
              console.log("["+today.toLocaleTimeString()+"][Content Database Log] Unknown error.");

            }
          });
			  } else {
			    console.log("["+today.toLocaleTimeString()+"][Content Database Log] Database Failed to Initialize.");
			  }
			});
            ...
            socket.on('photo_upload', function(details){
                  ...
                  console.log('\t[media_request] MID: ' +details);
									content.db.query('SELECT data from media where mid = ?', details, function(err, rows, fr) {
										if(err == null){
                      if(rows.length > 0){
                        var size = (rows[0].data.length)/1000);
                        socket.emit('media_request_data', size);
                        console.log('\t               SENT - Size: ' + (size/1000) + ' Kbytes');

                        socket.on('media_datagram_request', function(details){
  ...
  initLoginServer:function() {
      login_server.on('connection', function(socket){
            ...
            socket.on('user_create_request', function(details){
                  var id = dataManager.generateUID()
                  console.log('\t[user_create_request] Login Create request: ' + details.username);
                      ...
                      socket.emit("user_create_request_succeed", user.uid);
										} else {
...
var dataManager = {
  ...
  validChar:"1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_",

  generateUID: function() {
    var id = "";
    for(var i = 0; i <=11; i++){
      id += dataManager.validChar.charAt(Math.floor(Math.random()*65));
    }
    content.db.query('SELECT uid FROM user WHERE uid = ?', id , function(err, rows, fields) {
      if(err == null){
        if(rows.length == 0){
          return id;
        } else {
          return dataManager.generateUID():
        }
      }
    });
  },
  ...
  generateMID: function() {
    ...
    content.db.query('SELECT mid FROM media WHERE mid = ?', id , function(err, rows, fields) {
      if(err == null){
        if(rows.length == 0){
          return id;
        } else {
          return dataManager.generateMID():
        }
      }
    });
  }
